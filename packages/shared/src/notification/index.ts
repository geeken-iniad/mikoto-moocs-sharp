import { PERIODS } from "../schedule/constants";
import type {
  Course,
  ExceptionEntry,
  Period,
...
import { parseTimeSlotKey } from "../utils/schedule";
...
function getCancellationForSlot(
  schedule: Schedule,
  slotId: string,
  date: string,
): ExceptionEntry | undefined {
  return schedule.exceptions[date]?.find(
    (exception) =>
      exception.slotId === slotId && exception.type === "cancellation",
  );
}
...
export function getNextClass(
  ...
  for (const [timeSlotKey, slotId] of Object.entries(schedule.grid)) {
    ...
    const slotCancellationCount = Object.values(schedule.exceptions)
      .flat()
      .filter(
        (exception) =>
          exception.slotId === slotId && exception.type === "cancellation",
    ...
    for (
      let skippedWeeks = 0;
      skippedWeeks <= slotCancellationCount;
      skippedWeeks += 1
    ) {
      const cancellation = getCancellationForSlot(
        schedule,
        slotId,
        formatLocalDate(classDate),
      );
